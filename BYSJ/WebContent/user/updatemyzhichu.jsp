 ...
 <div class="boxcenter" style="overflow:visible;">
    <div class="yxj_zhuce" style="overflow:visible;">
       ...
       <div class="yxj_zc_con" style="overflow:visible;height: 450px;">
          ...
          <div class="yxj_zx01">
             <h1>编辑${querySpendingBills.spendingBillsName}账单：</h1>

        
             <span style="color: #ff0000;font-size: 14px;" id="message"></span>
          </div>
        <form  action="/BYSJ/spendingbills/updateUserSpendingBills" method="post" class="yxj_zx02" style="overflow:visible; height:400px;" onsubmit="return expertSent()">
           
             <div class="yxj_zx02L" style="overflow:visible;">
             	 <input type="hidden" name="spendingBillsId" value="${querySpendingBills.spendingBillsId}">
             	 <input type="hidden" name="uid" value="${user.userID}">
                 <h1><p>账单名称</p><span> 3大于字数小于8</span></h1>
                  <input type="text" id="spendingBillsName" name="spendingBillsName" placeholder=" ${querySpendingBills.spendingBillsName}">
                  <h1><p>账单类型</p><span>吃饭/旅游/逛街/其他</span></h1>
                  <input type="text" id="spendingBillsState" name="spendingBillsState" placeholder="${querySpendingBills.spendingBillsState}">
                  <h1><p>收入金额</p><span>RMB</span></h1>
                  <input type="text" id="spendingBillsMoney" name="spendingBillsMoney" placeholder="${querySpendingBills.spendingBillsMoney}">
                  
             </div>
             <div class="yxj_zx02R" style="overflow:visible;">
                
				<h1><p>账单内容</p><span>账单详情</span></h1>
				<textarea rows="5" cols="19" id="spendingBillsBody" name="spendingBillsBody"  style="resize: none; height:200px; width:320px; font-family:'黑体'; " placeholder="${querySpendingBills.spendingBillsBody}" ></textarea>		
...
<script type="text/javascript" >
function expertSent() {
    var spendingBillsName = $("#spendingBillsName").val();
    var spendingBillsState = $("#spendingBillsState").val();
    var spendingBillsMoney = $("#spendingBillsMoney").val();
    var spendingBillsBody = $("#spendingBillsBody").val();
    var number=/^(0|[1-9]\d*)$/;//判断是否是数字
    if(spendingBillsName.length<3 ) {
        $("#message").html('请输入正确账单名称！');
        $("#spendingBillsName").addClass("yxj_error");
        $("#spendingBillsName").focus();
        $('html,body').animate({scrollTop:$('#message').offset().top}, 800);
        return false;
    }
    if(spendingBillsName.length>8 ) {
        $("#message").html('请输入正确账单名称！');
        $("#spendingBillsName").addClass("yxj_error");
        $("#spendingBillsName").focus();
        $('html,body').animate({scrollTop:$('#message').offset().top}, 800);
    ...
    if("吃饭"!=spendingBillsState && "旅游"!=spendingBillsState && "逛街"!=spendingBillsState && "其他"!=spendingBillsState){
    	$("#message").html('请输入正确账单类型');
        $("#spendingBillsState").addClass("yxj_error");
        $("#spendingBillsState").focus();
        $('html,body').animate({scrollTop:$('#message').offset().top}, 800);
        return false;
    }

    if(!number.test(spendingBillsMoney)) {
        $("#message").html('请输入账单金额');
        $("#spendingBillsMoney").addClass("yxj_error");
        $("#spendingBillsMoney").focus();
        $('html,body').animate({scrollTop:$('#message').offset().top}, 800);
        return false;
    }
    if(!spendingBillsBody) {
