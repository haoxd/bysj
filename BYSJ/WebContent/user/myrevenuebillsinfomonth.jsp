		...
		<div class="head-body-wrap">
			<div class="head-body clearfix">
				<div class="fund-logo-wrap">
					...
					<div class="fund-slogan">E理财家庭理财网 <a href="http://localhost:8080/BYSJ/user/addmyshouru.jsp" target="_blank" title="新增收入账单" style="color: orange;">[我的收入/月账单]</a></div>
				</div>
				...
				<c:choose>
<c:when test="${empty userToRevenueBillsMonth}">
<p align="center" ><a style="color: orange; " href="http://localhost:8080/BYSJ/user/addmyshouru.jsp" target="_blank"><font size="5px">没有任何关于您的收入账单，请添加</font></a></p>
</c:when>
	...
	<div class="main-wide-wrap" data-sk-area="FundList-ska">
	 ...
	 <c:forEach items="${userToRevenueBillsMonth}" var="userToRevenueBills" > 
	...
	<!--###############################################  -->	
		...
		<c:choose>
      	...
      	<c:otherwise>
      		<a  style="color: orange;" href="/BYSJ/revenuebills/queryUserRevenuebillsThisMonth?pageNo=1&&uid=${user.userID}">首页</a>&nbsp;
      		<a   style="color: orange;"href="/BYSJ/revenuebills/queryUserRevenuebillsThisMonth?pageNo=${pb.pageNo-1}&&uid=${user.userID}">上一页</a>&nbsp;
      	</c:otherwise>
      </c:choose>
    	...
    	<c:choose>
    		...
    		<c:otherwise>
    			<a  style="color: orange;" href="/BYSJ/revenuebills/queryUserRevenuebillsThisMonth?pageNo=${pb.pageNo+1}&&uid=${user.userID}">下一页</a>&nbsp;
    			<a   style="color: orange;" href="/BYSJ/revenuebills/queryUserRevenuebillsThisMonth?pageNo=${pb.totalPage}&&uid=${user.userID}">尾页</a>&nbsp;
    		</c:otherwise>
    	</c:choose>
